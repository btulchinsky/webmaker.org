module.exports = function (init) {
    ...
    change: function (req, res) {
      ...
      fetch_event(req, function (event, isAdmin) {
        ...
        if (isAdmin) {
          ...
          allowed.push('organizerId'); 
          allowed.push('organizer'); 
        }

        if (isAdmin && changes.organizerId) {
          //need to get user email before updating the event organizer
          ctx.loginAPI.getUserByUsername(changes.organizerId, function (err, user) {
            //user was not found - for now, still update everything else except 
            //organizer info which we remove
            if (err || !user) { 
              delete changes.organizerId; 
            }
            else {
              changes.organizerId  = user.username;
              changes.organizer = user.email;
            }
            updateEvent();
          });
        }
        //regular update if the user is not an admin
        else { 
          //fix as even if organizerId is not in the allowed list, it can still get changed
  ...
  function picture_handler(picture, cb) {
    return function (event) {
      if (picture) {
        s3.put(picture.data, picture.type, function (error, result) {
          // check for S3 upload failure
          if (error) {
            cb(error);
          }

          var url = result && result.url;
          if (url) { // // ensure we recieved picture url in result
            var previousPicture = event.picture;
            if (previousPicture) {
              s3.delete(previousPicture);
            }
      ...
      } else { // no picture passed to picture_handler, continue on
        cb(null, event);
      }
    };
  }
